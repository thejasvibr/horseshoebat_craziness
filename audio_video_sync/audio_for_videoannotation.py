...
def search_for_best_fit(upsampled_video_sync, audio_files_to_search,
    ...
    print('Starting refined search within best doublechunk segment...')
...
def do_refined_search(audio_files_to_search, upsampled_video_sync,
    ...
    if spikey:
        ...
        cc = signal.correlate(double_chunk_audio, 
                              upsampled_video_sync,
                                 'same')
    ind = np.argmax(cc)
    shift = int(np.round((ind - upsampled_video_sync.size/2.0)))
    best_matching_audio = double_chunk_audio[shift:shift+upsampled_video_sync.size, :]
    return(best_matching_audio)
...
def parse_doublechunk_ids(doublechunk_ids, full_audio_filepaths, 
    ...
    filenames = [name.split('_')[0] for name in chunk_names]
    chunk_numbers = [name.split('_')[1] for name in chunk_names]         
...
if __name__ == '__main__':
    video_sync = pd.read_csv('common_fps_video_sync456.0.csv')
    #audio_folder = '/media/tbeleyur/THEJASVI_DATA_BACKUP_3/fieldwork_2018_002/horseshoe_bat/audio/2018-08-16'
    audio_folder = 'test_data/'
    kwargs = {}
    best_audio = match_video_sync_to_audio(video_sync, audio_folder,
                                                                 **kwargs)
    sf.write('matching_sync456.WAV', best_audio,250000)
